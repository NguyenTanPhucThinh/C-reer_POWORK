...
| **TC_EVAL_001** | Gửi criterion của Challenge khác | Body chứa `criteria_id` không thuộc Challenge của Submission. | Chấm điểm bằng sai rubric. | - HTTP `400` với `ASSESS_007`.<br>- Không tạo Evaluation Result.<br>- Không đổi Submission status. |
| **TC_EVAL_002** | Gửi điểm ngoài giới hạn | `score < 0`, `score > max_score` hoặc giá trị không hữu hạn. | Điểm không hợp lệ làm sai kết quả và snapshot. | - HTTP `400` với `ASSESS_007`.<br>- Chấp nhận đúng hai biên `0` và `max_score`.<br>- Không ghi dữ liệu khi vi phạm. |
| **TC_EVAL_003** | Gửi trùng criterion trong một request | Mảng `evaluations` chứa cùng `criteria_id` nhiều lần. | Một tiêu chí được cộng điểm nhiều lần. | - HTTP `400` với `ASSESS_007`.<br>- Toàn bộ request bị từ chối atomically. |
| **TC_EVAL_004** | Chấm lại Submission không còn Pending | Gọi `/evaluate` cho Submission `Evaluated`, `Approved` hoặc `Rejected`. | Tạo nhiều bộ điểm hiệu lực. | - HTTP `409` với `ASSESS_011`.<br>- Unique `(submission_id, criteria_id)` bảo vệ thêm ở database.<br>- Không đổi dữ liệu cũ. |
| **TC_EVAL_005** | Unlock Submission chưa được chấm hợp lệ | Submission chưa `Evaluated` hoặc không có Evaluation Result. | Tạo snapshot rỗng hoặc sai điểm. | - HTTP `409` với `ASSESS_011`.<br>- Không đổi `is_unlocked`.<br>- Không tạo Verified Evidence. |
| **TC_EVAL_006** | Hai request unlock đồng thời | Cả hai request cùng đọc `is_unlocked = false`. | Tạo hai snapshot hoặc thay đổi ngược kết quả đã chốt. | - Chỉ một compare-and-set thành công.<br>- Request còn lại nhận `409 ASSESS_004`.<br>- Unique `source_hash_id` bảo đảm chỉ một snapshot tồn tại. |
| **TC_UNLOCK_001** | Gọi unlock lặp lại | Cùng Submission đã unlock được gọi `/unlock` lần hai. | Tạo Evidence trùng hoặc trả danh tính lần nữa như một thao tác mới. | - HTTP `409 ASSESS_004`.<br>- Chỉ một Verified Evidence tồn tại.<br>- Submission giữ `Approved`. |
| **TC_UNLOCK_002** | Một bước cuối transaction thất bại | IAM lookup hoặc tạo Evidence ném lỗi sau khi compare-and-set. | `is_unlocked` hoặc Submission bị commit một phần. | - Toàn bộ transaction rollback.<br>- `is_unlocked = false`.<br>- Submission trở lại `Evaluated`.<br>- Không có Verified Evidence. |
| **TC_UNLOCK_003** | Employer thêm Candidate do công ty khác unlock | JWT thuộc Company A, Candidate chỉ được unlock qua Challenge của Company B. | Dùng `user_id` đã lộ hợp lệ để chiếm Candidate vào Talent Pool. | - HTTP `403 POOL_007`.<br>- Không lookup thêm dữ liệu Candidate.<br>- Không tạo Talent Pool entry. |
| **TC_UNLOCK_004** | Employer thêm Candidate mình đã unlock | JWT và Challenge đã unlock cùng thuộc một công ty. | Guard từ chối nhầm Candidate hợp lệ. | - HTTP `201 Created`.<br>- Talent Pool entry dùng `company_id` từ JWT.<br>- Không nhận `company_id` từ request body. |
